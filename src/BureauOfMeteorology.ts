...
module.exports.getAlerts = async function getAlerts(state) { // Bureau of Meteorology has a list per state, so we can check if the state is valid.
    ...
    const feed = await parser.parseURL(`https://reg.bom.gov.au/fwo/IDZ${id}.warnings_${state}.xml`);
    const alerts = [];
    feed.items.forEach(item => {
        ...
        // Severity will either be "Minor", "Moderate", "Major", or "Final". How do we know which one it is? Well, we can check the title.
        // It seems like "Final" alerts are "sending off" alerts, but I'll leave it in just in case.
        let severity = "";
        if (item.title.includes("Minor")) severity = "Minor";
        if (item.title.includes("Moderate")) severity = "Moderate";
        if (item.title.includes("Major")) severity = "Major";
        if (item.title.includes("Final")) severity = "Final";
